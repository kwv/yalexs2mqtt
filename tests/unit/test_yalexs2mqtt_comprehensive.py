...
with patch("builtins.open", create=True) as mock_open, \
    ...
    from yalexs2mqtt import Yalexs2MqttBridge, DisconnectedError
...
def bridge(mock_client):
    # We need to mock open/json.load again for the class instantiation if it loads config in init
...
def test_new_state_publishing(bridge):
    ...
    bridge.mqtt_client.publish.assert_called_with(
        "yalexs/ASDFGH1234/currentValue", 
        status_json, 
        retain=True
    )
...
async def test_run_loop_lock_command(bridge, mock_push_lock, mock_scanner):
    ...
    with patch("asyncio.wait", side_effect=side_effect_wait), \
         patch("sys.exit"):
        try:
            await bridge.run()
...
async def test_disconnected_error_handling(bridge, mock_push_lock, mock_scanner):
    ...
    with patch("sys.exit") as mock_exit, \
        ...
        mock_log.assert_called_with("Disconnected Error: Disconnected. Check your handshakeKey and handshakeKeyIndex in config.json.")
